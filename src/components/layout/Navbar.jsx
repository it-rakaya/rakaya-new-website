  ...
  return (
    <nav class="navbar navbar-expand-lg bg-whtie shadow-sm py-3">
      <Container className='col-lg-10 align-items-center'>
        ...
        <button class="btn fs-1 border-0 d-lg-none" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation"  ref={buttonRef} onClick={() => { setToggleNav(prev => !prev) }}>
          <i className={`${buttonRef.current && !buttonRef.current?.classList.contains('collapsed') ? "bi bi-x" : "bi bi-list"}`}></i>
        </button>
